...
describe("formatting.smart-quotes", () => {
  ...
  it("passes curly quotes", () => {
    ...
    const packet = makePacket({ tossups: [t], allParagraphs: t.paragraphs });
    const diags = lint(packet);
    const smartDiags = diags.filter((d) => d.rule === "formatting.smart-quotes" && d.message.includes("typographic"));
    // The first smart-quotes diagnostic about double quotes should not fire
    expect(smartDiags.some((d) => d.message.includes("straight quotes"))).toBe(false);
  });
});
...
describe("formatting.bce-ce-system", () => {
  it("flags BC usage", () => {
    const t = tossupWith("This battle occurred in 490 BC. For 10 points, name it.");
    const packet = makePacket({ tossups: [t], allParagraphs: t.paragraphs });
    const diags = lint(packet);
  ...
  it("flags AD usage", () => {
    const t = tossupWith("This event occurred in AD 476. For 10 points, name it.");
    const packet = makePacket({ tossups: [t], allParagraphs: t.paragraphs });
    const diags = lint(packet);
    expect(hasDiag(diags, "formatting.bce-ce-system")).toBe(true);
  });
  ...
  it("passes BCE/CE", () => {
    const t = tossupWith("This battle occurred in 490 BCE. For 10 points, name it.");
    const packet = makePacket({ tossups: [t], allParagraphs: t.paragraphs });
    const diags = lint(packet);
