...
describe('packet.numbering-sequence', () => {
  it('fires when tossup numbers decrease', () => {
    ...
    const t2 = makeQuestion(
      'tossup',
      3,
      'Q3. For 10 points, name it.',
      'ANSWER: **__a2__**',
      { numberParagraphIndex: 20 }
    );
    const t3 = makeQuestion(
      'tossup',
      2,
      'Q2. For 10 points, name it.',
      'ANSWER: **__a3__**',
...
describe('question.bonus-part-order', () => {
  it('fires when bonus parts are not interleaved with answers', () => {
    const leadinPara = makeParagraph(
      '1. Answer the following for 10 points each.',
      { index: 100 }
    );
    ...
    expect(findDiag(diags, 'question.bonus-part-order')!.severity).toBe(
      'error'
    );
  });
...
describe('answerline.no-nonstandard-prefix', () => {
  it("fires on 'Ans:' in non-answer paragraphs", () => {
    ...
    expect(findDiag(diags, 'answerline.no-nonstandard-prefix')!.severity).toBe(
      'error'
    );
  });
...
describe('question.multiline-answer', () => {
  it('fires when text follows an answer line', () => {
    ...
    expect(findDiag(diags, 'question.multiline-answer')!.severity).toBe(
      'error'
    );
  });
