...
function validBonus(n: number) {
  return makeQuestion(
    'bonus',
    n,
    'Answer the following about stuff for 10 points each.',
    '',
    {
      tag: '<Auth, Biology>',
...
describe('packet.section-headers', () => {
  ...
  it('flags missing bonus header', () => {
    ...
    expect(
      diags.some(
        (d) =>
          d.rule === 'packet.section-headers' && d.message.includes('Bonuses')
      )
    ).toBe(true);
  });
  ...
  it('flags bold question numbers', () => {
    const tossup = makeQuestion(
      ...
      'ANSWER: x',
      {
        ...
        numberRuns: [
          {
            text: '1. For 10 points, name this.',
            bold: true,
            italic: false,
            underline: false,
            superscript: false,
...
describe('packet.expected-count', () => {
  ...
  it('warns when not 20 bonuses', () => {
    ...
    expect(
      diags.some(
        (d) => d.rule === 'packet.expected-count' && d.message.includes('bonus')
      )
    ).toBe(true);
  });
});
