...
describe('Rule Registry', () => {
  ...
  it('every registry rule ID is produced by the lint engine', () => {
    ...
    const tossup = makeQuestion(
      'tossup',
      1,
      'This "test" has issues, for ten points, name it.',
      'answer no formatting',
      {
    ...
    const bonus = makeQuestion(
      'bonus',
      1,
      'Answer some questions about testing for 10 points each.',
      '',
      {
        tag: '<Bad Tag',
        parts: [
    ...
    for (const ruleId of producedRules) {
      ...
      expect(
        inRegistry,
        `Rule "${ruleId}" produced by engine but not in registry`
      ).toBe(true);
    }
  });
  ...
  it('all categories are valid', () => {
    ...
    for (const rule of RULE_REGISTRY) {
      expect(
        ...
        `Invalid category "${rule.category}" for rule ${rule.id}`
      ).toBe(true);
    }
  });
  ...
  it('all severities are valid', () => {
    ...
    for (const rule of RULE_REGISTRY) {
      expect(
        validSeverities.has(rule.defaultSeverity),
        `Invalid severity "${rule.defaultSeverity}" for rule ${rule.id}`
      ).toBe(true);
    }
  });
});
